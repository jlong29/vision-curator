...
def build_parser() -> argparse.ArgumentParser:
    ...
    score_parser.add_argument("--store-root", required=True)
    ...
    queue_parser.add_argument("--store-root", required=True)
...
def _ingest_package(args: argparse.Namespace) -> None:
    record = ingest_package(args.source, args.store_root)
    print(json.dumps(record.to_dict(), sort_keys=True))


def _score_package(args: argparse.Namespace) -> None:
    scores = score_package(args.package_id, args.store_root)
    output = Path(args.store_root) / "scores" / args.package_id / "track_scores.parquet"
...
def _build_review_queue(args: argparse.Namespace) -> None:
    queue_id, output, items = build_review_queue(args.queue_kind, args.store_root, args.limit)
